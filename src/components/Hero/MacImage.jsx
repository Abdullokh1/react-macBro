...
export default function MacImages(){
  return (
    <>
      <div className='img-wrapper me-5'>
        <ul className='d-flex main-list'>

            {obj[0].gold.map((item,i) =>{
              return (
                <>
                  <li key={i}>
                    <img className="main-img" src={item} width='450' height='430' />
                  </li>
                </>
              )
            })}
        ...
        <ul className='d-flex '>
            {obj[0].gold.map((item, i) =>{
              return (
                ...
                <li key={i}>
                  <button className='mac-img-btn'>
                      <img className="macbook-img" src={item} width='90' height='90' />
                  </button> 
                </li>
...
window.addEventListener('load', () =>{
  const btns = document.querySelectorAll('.mac-img-btn')

  btns.forEach((item, index) =>{
    item.addEventListener('click', ()=>{
      const list = document.querySelector('.main-list')
      list.style.transform = `translateX(${-index * 400}px)`
    })
  })
})
